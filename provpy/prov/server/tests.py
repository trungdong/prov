...
class AuthenticationTest(unittest.TestCase):
    ...
    def testApiKeyAuth(self):
        ...
        response = self.client.get('/api/v0/container/?format=json', **{'HTTP_AUTHORIZATION': auth})
        ...
        response = self.client.get('/api/v0/container/?format=json', **{'HTTP_AUTHORIZATION': fake_auth})
        self.assertEqual(response.status_code, 401)
        
    def testAnonymousAuth(self):
        logging.debug('Executing GET method with anonymous user...')
        response = self.client.get('/api/v0/container/?format=json')
        self.assertEqual(response.status_code, 200)
    ...
    def testUserPermissions(self):
        ...
        response = self.client.post('/api/v0/container/',data=data,content_type='application/json',
        ...
        destination = '/api/v0/container/' + `bundle.id` + '/?format=json'
