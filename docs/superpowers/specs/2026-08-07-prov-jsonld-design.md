...
| `@context` on output | **Reference the canonical context URL by default; embed opt-in** | Matches the submission's own examples and ProvToolbox output; smaller documents. `context="embed"` inlines the vendored §5 context for offline/self-contained use. *(Supersedes the roadmap step 40 note's lean toward embed-by-default — decided 2026-08-07.)* The submission schema's Context production cannot describe an embedded context, so schema validation applies to url-mode output only (ruled 2026-08-07); embed-mode validity is covered by the pyld expansion tests. |
