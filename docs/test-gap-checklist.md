...
- [x] `read(source)` without a format auto-detects by trying each registered deserializer in turn — one test per detectable format (json, xml, rdf) — **T12** (finding: json and rdf/trig genuinely auto-detect; an xml document does not — the registry tries rdf, as trig, before xml, and rdflib's `BadSyntax` on the xml content is a `SyntaxError` that read()'s except clause doesn't catch, so it propagates before xml is ever tried. Test pins this actual behaviour rather than asserting the aspirational "all three auto-detect") — **superseded in 2.5.0 (#239, PR #265): all three formats now auto-detect; auto-detection swallows all deserializer errors and treats an empty parse as "not this format"**
- [x] `read()` on undetectable/garbage input raises `TypeError` with the "specify the format" message after exhausting all serializers — **T12** (finding: unreachable via any real file — `ProvNSerializer.deserialize()` unconditionally raises uncaught `NotImplementedError` and is tried third, before the loop can ever exhaust normally; test reaches the branch by mocking all four deserializers to raise a caught exception type) — **superseded in 2.5.0 (#239, PR #265): auto-detection now catches all exceptions, so the `TypeError` is reachable with real undetectable input**
- [x] `read()` accepts both a filename (`str`/`PathLike`) and a file object, matching `ProvDocument.deserialize` — **T12**
- [x] Auto-detection swallows only `(TypeError, ValueError, AttributeError, KeyError)` from candidate deserializers — **T13** (`test_read.py::test_read_auto_detect_only_swallows_the_documented_exception_types` mocks a deserializer to raise `RuntimeError` and asserts it propagates instead of being swallowed) — **superseded in 2.5.0 (#239, PR #265): auto-detection now swallows all exceptions from candidate deserializers; the narrow-catch test was replaced accordingly**
