        ...
        def _add_bundle(bundle):
            ...
            if use_labels:
                if bundle.label == bundle.identifier:
                    ...
                    # Fancier label if both are different. The label will be the main node text, whereas the identifier
                    # will be a kind of subtitle.
                    bundle_label = f'<{bundle.label}<br />' \
                                   f'<font color="#333333" point-size="10">{bundle.identifier}</font>>'
        ...
        def _add_node(record):
            ...
            if use_labels:
                if record.label == record.identifier:
                    ...
                    # kind of subtitle.
                    node_label = f'<{record.label}<br />' \
                                 f'<font color="#333333" point-size="10">{record.identifier}</font>>'
            else:
