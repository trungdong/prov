...
does not produce byte-for-byte matching output for every file, for two distinct reasons,
both listed in `KNOWN_DIFFERENCES`:
...
**Datatype-palette drift** (79 files) - the generator's shared `addTypes()`/`addLocations()`
helper (`ProvFrameworkTest.java`) does not cover `xsd:gMonth`, `xsd:yearMonthDuration` or
`xsd:dayTimeDuration`, each of which the PROV-JSON fixture's datatype/location list carries
six times, so these files' position in an indexed demonstration series (or their combined
type/location list) no longer lines up with the JSON fixture of the same name:
